...
const Experience = () => {
  ...
  return (
    <section id="experience" className="py-24 relative bg-warm-ambient bg-dot-pattern">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          {experience.map((exp, idx) => (
            ...
            >
              {/* Header Info */}
              <div className="flex flex-wrap items-start justify-between gap-4 border-b border-[#1F2442] pb-6">
                <div>
                  <span className="px-3.5 py-1 rounded-full bg-[#8B5CF6]/20 border border-[#8B5CF6]/40 text-[#EC4899] text-xs font-sans font-bold uppercase tracking-wider mb-2 inline-block">
                    DATA ANALYTICS INTERNSHIP
                  </span>
                  <h3 className="text-2xl sm:text-3xl font-display font-bold text-[#F8FAFC]">
                    {exp.role}
                  </h3>
                  <div className="text-lg font-display italic text-[#EC4899] mt-1 font-semibold">
                    {exp.company}
                  </div>
                </div>

                <div className="space-y-1 text-left sm:text-right font-sans text-xs text-[#CBD5E1]">
                  <div className="flex items-center space-x-1.5 justify-start sm:justify-end text-[#F8FAFC] font-bold">
                    <Calendar size={14} className="text-[#8B5CF6]" />
                    <span>{exp.period}</span>
                  </div>
                  <div className="flex items-center space-x-1.5 justify-start sm:justify-end text-[#CBD5E1]">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {exp.deliverables.map((item, iIdx) => (
                    <div
                      key={iIdx}
                      className="p-4 rounded-2xl bg-[#070A18] border border-[#1F2442] space-y-2 hover:border-[#8B5CF6] transition-all"
                    >
                      <div className="flex items-center space-x-2 text-[#F8FAFC] font-sans font-semibold text-xs">
                        <CheckCircle2 size={14} className="text-[#8B5CF6] shrink-0" />
                        <span>{item.title}</span>
                      </div>
                      <p className="text-xs text-[#CBD5E1] leading-relaxed font-sans font-normal">
                        {item.desc}
                      </p>
                    </div>
                  ))}
              ...
              {exp.photos && exp.photos.length > 0 && (
                <div className="space-y-4 pt-4 border-t border-[#1F2442]">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-sans font-bold text-[#F8FAFC] uppercase tracking-wider flex items-center space-x-2">
                    ...
                    {exp.photos.map((photo, pIdx) => {
                      ...
                      return (
                        <div
                          ...
                          className="group relative rounded-2xl overflow-hidden border border-[#1F2442] bg-[#070A18] aspect-4/3 cursor-pointer shadow-xs hover:border-[#EC4899] transition-all"
                        >
                          <img
                            src={url}
                            alt={title}
                          ...
                          />
                          <div className="absolute inset-0 bg-[#070A18]/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-2 text-center">
                            <span className="px-3 py-1 rounded-full bg-[#8B5CF6] text-white text-[11px] font-sans font-bold flex items-center space-x-1 shadow-md">
                              <Eye size={12} />
                              <span>View</span>
                            </span>
                          </div>
          ...
          {selectedPhoto && (
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-[#070A18]/85 backdrop-blur-md">
              <motion.div
                ...
                className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl bg-[#101225] border border-[#1F2442] p-6 shadow-2xl text-[#F8FAFC]"
              >
                <button
                ...
                <div className="space-y-4">
                  ...
                  <div className="rounded-2xl overflow-hidden border border-[#1F2442] bg-black/20 flex items-center justify-center max-h-[60vh]">
                    <img
                  ...
                  <div className="pt-4 border-t border-[#1F2442] flex justify-between items-center">
                    <a
                    ...
                    <button
                      onClick={() => setSelectedPhoto(null)}
                      className="px-4 py-2 rounded-full bg-[#070A18] text-[#F8FAFC] border border-[#1F2442] font-sans font-semibold text-xs"
                    >
                      Close
                    </button>
                  </div>
